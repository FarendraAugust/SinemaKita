<?php
include 'utils/connection.php'; // sudah otomatis ambil .env dari connection.php
  ...
  <main class="flex-1 p-6">
    ...
    <header class="flex justify-between items-center mb-6">
      ...
      <button 
        ...
        onclick="window.location.href='<?= $base_url ?>/admin/tag/tambah'">
        Tambah Tag
      </button>
    </header>
    ...
    <div class="bg-[#18191c] rounded-lg shadow overflow-x-auto p-4">
      <table id="tagTable" class="w-full text-left border-collapse">
        ...
        <tbody class="divide-y divide-gray-700 text-white">
          <?php
          $no = 1;
          $data = mysqli_query($connection, "SELECT * FROM tag ORDER BY id_tag DESC");
          while ($d = mysqli_fetch_array($data)) { ?>
            <tr class="odd:bg-[#1b1c1f] even:bg-[#222325] hover:bg-[#2a2b2e] transition">
              <td class="px-6 py-3"><?= $no++; ?></td>
              <td class="px-6 py-3"><?= htmlspecialchars($d['nama_tag']); ?></td>
              <td class="px-6 py-3"><?= htmlspecialchars($d['klik']); ?></td>
              ...
              <td class="px-6 py-3 text-center flex space-x-2">
                <button 
                  ...
                  onclick="window.location.href='<?= $base_url ?>/admin/tag/edit/<?= $d['id_tag'] ?>'">
                  ✏️
                </button>
                ...
                <button 
                  ...
                  data-id="<?= htmlspecialchars($d['id_tag']); ?>"
                  data-nama="<?= htmlspecialchars($d['nama_tag']); ?>">
                  🗑️
                </button>
              </td>
            </tr>
...
<div id="modalHapus"
  ...
  <div class="bg-[#18191c] text-white rounded-lg shadow-lg w-full max-w-md p-6">
    ...
    <p class="mb-4">
      ...
      <span id="namaTag" class="font-semibold text-red-400"></span>?
    </p>

    <form id="formHapus" method="POST" 
          action="<?= $base_url ?>/src/peges/admin_dashboard/tag/hapus_action.php" 
          class="mt-6 flex justify-end gap-3">
      <input type="hidden" name="id_tag" id="idTag">
...
<script>
  $(document).ready(function () {
    ...
    // event tombol hapus
    $('#tagTable').on('click', '.btn-hapus', function () {
